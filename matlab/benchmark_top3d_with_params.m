...
function benchmark_top3d_with_params(nelx, nely, nelz, volfrac, penal, rmin, output_file, maxloop, tolx)
...
%   maxloop: Maximum number of iterations (optional, default=200)
%   tolx: Termination criterion (optional, default=0.01)

% Handle default parameters
if nargin < 8
    maxloop = 200;    % Default maximum number of iterations
end
if nargin < 9
    tolx = 0.01;      % Default termination criterion
end
...
fprintf('Parameters: maxloop=%d, tolx=%.4f\n', maxloop, tolx);
tic;
...
memory_tracking = zeros(maxloop, 1);

% START ITERATION
while change > tolx && loop < maxloop
    loop = loop+1;
    
    % Track memory at this iteration
    memory_tracking(loop) = get_memory_usage() - memory_before;
    ...
    % For very large problems, check if we should terminate early due to memory constraints
    if memory_tracking(loop) > 10000  % 10GB threshold
        fprintf('WARNING: Memory usage exceeding 10GB, terminating early\n');
        break;
    end
    
    % For very large problems, limit iterations with little change
    if loop > 20 && change < 3*tolx
        fprintf('Early convergence criteria met (change < 3*tolx for loop > 20)\n');
        break;
    end
end
...
memory_used = max(memory_tracking(1:loop));  % Use peak memory during iteration
if memory_used == 0
    memory_used = memory_after - memory_before;  % Fallback if tracking failed
end
